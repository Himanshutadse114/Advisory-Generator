...
function LayerGroup({ id, layer, selected, interactive, onSelect, onMove, children }) {
  ...
  const handlePointerDown = event => {
    ...
    dragRef.current = { start: pointToSvg(event), pointerId: event.pointerId }
  ...
  const finishDrag = event => {
    ...
    const delta = drag; dragRef.current = null; setDrag({ x: 0, y: 0 })
...
const AdvisoryCanvas = forwardRef(function AdvisoryCanvas({ advisory, template = 'editorial', brand = getBrandProfile('innvikta'), editor, visualFamily = '', interactive = false, onSelectLayer, onMoveLayer }, ref) {
  const shared = { advisory, brand, editor, interactive, onSelectLayer, onMoveLayer, visualFamily }
  return <svg ref={ref} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label={`${advisory.title} advisory preview`} style={{ width: '100%', height: 'auto', display: 'block', touchAction: 'none' }}><rect width={WIDTH} height={HEIGHT} fill={brand.paper} />{template === 'split' && <SplitTemplate {...shared} />}{template === 'flow' && <FlowTemplate {...shared} />}{template === 'editorial' && <EditorialTemplate {...shared} />}</svg>
})
